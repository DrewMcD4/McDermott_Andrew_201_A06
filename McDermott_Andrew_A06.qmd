...
[https://github.com/DrewMcD4/McDermott_Andrew_201_A06](https://github.com/DrewMcD4/McDermott_Andrew_201_A06)
...
```{r}
height <- list(MJ = 78, LJ = 81, KB = 78, KA = 86, SO = 85)

for(i in names(player_info)){
  player_info[[i]] <- append(player_info[[i]], list(height = height[[i]]))
}
player_info
```
...
for(i in names(player_info)){
  ppg <- player_info[[i]][['points']]/player_info[[i]][['games_played']]
...
```{r}
points_per_game_rounded <- sapply(points_per_game, function(x){round(x,2)})
points_per_game_rounded
```
...
mat_rand_rounded <- apply(mat_rand, 1:2, rounding)
```
...
for(i in c('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec')){
  average_temp <- mean(max_temps[[i]])
  mean_temps[[i]] <- average_temp
}
...
```{r}
temps_co <- max_temps[max_temps$state == "CO", TRUE]
temps_oh <- max_temps[max_temps$state == "OH", TRUE]
temps_va <- max_temps[max_temps$state == "VA", TRUE]
```
...
```{r}
temps_co$state <- NULL #state names are redundant so removed
temps_co$annual_med <- apply(temps_co[,-1], 1, median) #exclude year column and calculate medians
max_year <- temps_co$year[[which.max(temps_co$annual_med)]]
min_year <- temps_co$year[[which.min(temps_co$annual_med)]]
max_year_temp <- temps_co$annual_med[[which.max(temps_co$annual_med)]]
min_year_temp <- temps_co$annual_med[[which.min(temps_co$annual_med)]]
paste0("The maximum median annual temperature of ", max_year_temp, " occurred in ", max_year, " and the minimum median annual temperature of ", min_year_temp, " occurred in ", min_year, ".")
```
...
sum_of_digits <- function(number){
  if(number == 0){
    ...
    #number %% 10 finds the remainder when divided by 10 - which is the last number
    #number %/% 10 gives the result before remainder - which is the remaining number after the last digit(remainder) is removed
    #calling sum_of_digits on number%/% repeats the function on the remaining number until all digits have been removed as remainders
  }
}
